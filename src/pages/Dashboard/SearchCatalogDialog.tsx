  ...
  const getCatalogs = async ({pageParam = 1}) => {
    try {
      const res = await fetch(
        `${import.meta.env.VITE_REST_SERVICE_BASE_URL}/catalog?title=${
          search ?? ''
        }&page=${pageParam}&amount=${4}`,
        {
          headers: {
      ...
      <DialogContent>
        ...
        <div id="catalogList" className="max-h-[300px] overflow-auto">
          <InfiniteScroll
            ...
            dataLength={catalogs ? catalogs.length : 0}
            scrollableTarget="catalogList"
            className="flex flex-col gap-4"
          >
            {catalogs &&
              catalogs.map(
                (
                  catalog: {uuid: string; title: string; poster: string},
                  index: number,
                ) => (
                  <div
                    key={index}
          ...
          {(!catalogs || catalogs.length === 0) && <div>No catalogs found.</div>}
        </div>
        <DialogClose asChild>
